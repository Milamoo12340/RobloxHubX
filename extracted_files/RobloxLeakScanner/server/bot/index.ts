...
export class BotService {
  ...
  /**
   * Start periodic health checks to ensure the bot is always running
   */
  private startHealthChecks() {
    ...
    this.healthCheckInterval = setInterval(async () => {
      ...
      if (!isActive) {
        console.log('Bot is inactive, attempting to restart...');
        await this.activate();
        
        // Verify the restart was successful
        const restartSuccess = await this.isActive();
        if (restartSuccess) {
          console.log('Bot successfully restarted');
        } else {
          console.error('Failed to restart the bot');
        }
      } else {
        console.log('Bot health check passed - bot is active');
      }
    }, HEALTH_CHECK_INTERVAL);
    
    console.log('Bot health check service started. Will run every 5 minutes.');
  }
  ...
  stopHealthChecks() {
    if (this.healthCheckInterval) {
      ...
      console.log('Bot health check service stopped.');
    }
  }
  ...
  async isActive(): Promise<boolean> {
    return !!this.autoDiscoveryInterval && this.bot.isConnected();
  }
  
  /**
   * Activate the bot if it's not already running
   */
